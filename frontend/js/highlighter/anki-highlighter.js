...
async function loadKnownBasicWords({ force = false } = {}) {
    if (knownBasicWordsLoaded && !force)
        return;
    try {
...
function isVerbChainTailToken(token) {
    ...
    if (pos === "助動詞")
        return true;
    if (pos === "助詞" && ["て", "で", "たら", "ても", "でも", "ば", "える"].includes(surface))
        return true;
    if (pos === "動詞" && ["しまう", "ちゃう", "いる", "ある", "くれる", "もらう", "くださる"].includes(basic)) {
        return true;
    }
    if (pos === "動詞" && posDetail1 === "接尾")
        return true;
    return false;
}
...
function rerenderCurrentSubtitleWithAnkiHighlighter() {
    if (typeof getCurrentSubtitle !== "function")
        return;
    if (typeof renderSubtitleOverlay !== "function")
        return;
...
function isLearnedAnkiStatusForComprehension(status) {
...
function getUnknownKanjiTokenCountForText(text) {
    ...
    for (const token of tokens) {
        ...
        const start = getTokenStart(token);
        const end = getTokenEnd(token);
        const coveredByLearnedMatch = learnedMatches.some((match) => match.start <= start &&
            match.end >= end);
