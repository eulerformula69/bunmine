class AudioTrackManager {
	...
	async setTrack(trackId: string, videoPayload: CurrentVideoPayload | null): Promise<void> {
		if (trackId === "default") {
			if (this.originalSource) await this.replaceSource(this.originalSource);
			this.currentTrack = "default";
			return;
		}
		...
		try {
			...
			if (!this.originalSource) this.originalSource = this.video.currentSrc || this.video.src;
	...
	async replaceSource(source: string): Promise<void> {
