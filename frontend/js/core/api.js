...
function getApiErrorMessage(data, fallback = "Request failed") {
    ...
    if (data.errorInfo?.message)
        return data.errorInfo.message;
    return fallback;
}
function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
async function fetchWithRetry(url, options, { retries = 5, delayMs = 800, label = "request" } = {}) {
    let lastError = null;
    for (let attempt = 1; attempt <= retries; attempt += 1) {
        try {
            ...
            lastError = err;
            console.warn(`${label} failed ${attempt}/${retries}:`, err instanceof Error ? err.message : String(err));
            if (attempt < retries) {
