...
function applyLibraryLanguage() {
    ...
    document.querySelectorAll("[data-i18n]").forEach((el) => {
        ...
        if (key)
            el.textContent = lt(key);
    });
    if (librarySummary?.textContent === "Loading..." || librarySummary?.textContent === "Загрузка..." || librarySummary?.textContent === "読み込み中...")
        librarySummary.textContent = lt("loading");
}
function watchedTextForEpisode(episode) {
    if (episode.completed)
        return lt("watched");
    if (episode.currentTimeSeconds > 0)
        return lt("atTime", { time: formatLibraryTime(episode.currentTimeSeconds) });
...
function recomputeSeriesLinkStatusFromEpisodes(episodes) {
    ...
    if (episodesWithVideo <= 0 && episodesWithSubtitle <= 0)
        return "missing";
    if (episodesWithVideo === items.length && episodesWithSubtitle === items.length)
        return "linked";
    return "partial";
}
function refreshCurrentSeriesLinkStatus() {
    if (!currentOpenedSeries)
        return;
...
function updateSeriesCompletedCount(seriesId, delta) {
    ...
    for (const card of cards) {
        if (String(card.dataset.seriesId) !== String(seriesId))
            continue;
...
function updateCurrentSeriesStatsText(delta) {
    if (!currentOpenedSeries)
        return;
    currentOpenedSeries.completedEpisodes = Math.max(0, Number(currentOpenedSeries.completedEpisodes || 0) + delta);
}
...
async function startAndPollLibraryJob(requestPath, requestOptions = {}, failureMessage = lt("scanFailed")) {
    ...
    while (true) {
        ...
        const statusJobData = statusData;
...
async function selectSubtitleResult(result) {
    ...
    try {
        ...
        if (btn)
            btn.textContent = lt("changeJpSubs");
...
function closeBulkSubtitleModal() {
    if (isBulkSubtitleDownloading || isBulkSubtitlePreparing)
        return;
...
async function analyzeMissingSubtitlesForCurrentSeries() {
    if (!currentOpenedSeries || isBulkSubtitlePreparing || isBulkSubtitleDownloading)
        return;
...
function getBulkSubtitleSets(plan) {
    ...
    for (const item of items) {
        ...
        for (const candidate of candidates) {
            ...
            if (!releaseKey || usedForEpisode.has(releaseKey))
                continue;
...
function getSelectedBulkSubtitleItems() {
    if (!currentBulkSubtitlePlan)
        return [];
    const selectedIds = new Set(Array.from(bulkSubtitleList.querySelectorAll(".bulk-subtitle-checkbox:checked"))
...
async function postSubtitleDownloadWithBackoff(item) {
    ...
    for (let attempt = 0; attempt <= JIMAKU_429_MAX_RETRIES; attempt += 1) {
        ...
        if (stateEl)
            stateEl.textContent = lt("rateLimitedRetrying", { seconds: Math.ceil(waitMs / 1000) });
        await sleep(waitMs);
    }
    throw new Error(lt("jimakuRetryReached"));
}
async function downloadSelectedBulkSubtitles() {
    ...
    async function worker(workerId) {
        while (nextIndex < items.length) {
            ...
            if (stateEl)
                stateEl.textContent = lt("downloadingState");
            try {
                await postSubtitleDownloadWithBackoff(item);
                downloaded += 1;
                if (stateEl)
                    stateEl.textContent = lt("downloadedState");
            }
            catch (err) {
                failed += 1;
                if (stateEl)
                    stateEl.textContent = lt("failedState", { message: err.message });
            }
...
async function deleteSeriesFromLibrary(seriesId, title) {
    ...
    try {
        ...
        if (!response.ok || data.error)
            throw new Error(data.error || lt("deleteSeriesFailed"));
...
async function relinkCurrentSeriesFiles() {
    ...
    try {
        ...
        if (!response.ok || data.error)
            throw new Error(data.error || lt("relinkFailed"));
