...
function renderSubtitleOverlay(options) {
    ...
    for (let position = 0; position < texts.length; position += 1) {
        ...
        if (cueIndex >= 0)
            line.dataset.subtitleIndex = String(cueIndex);
        if (cueIndex === primaryIndex)
...
function applySubtitleCueStyle(line, cue) {
    if (!cue)
        return;
    if (cue.fontName)
        line.style.fontFamily = `"${cue.fontName}", "NotoSansJP", sans-serif`;
    if (cue.fontSize)
        line.style.fontSize = `${Math.max(0.6, Math.min(2.5, cue.fontSize / 40))}em`;
    if (cue.primaryColor)
        line.style.color = cue.primaryColor;
    if (cue.bold)
...
function getSubtitleRegionKey(cue) {
    if (cue?.positionX !== undefined && cue?.positionY !== undefined)
        return `positioned-${cue.positionX}-${cue.positionY}`;
    const alignment = Number(cue?.alignment || 2);
    if (alignment >= 7)
        return "top";
    if (alignment >= 4)
