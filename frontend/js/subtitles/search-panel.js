function getSubtitleSearchDict() {
    return i18n?.[currentLang]?.dict || i18n?.en?.dict || {};
}
function ensureSubtitleSearchPanel(sidebarEl, state, callbacks) {
    if (!sidebarEl)
        return;
    ...
    if (document.getElementById("subtitleSearchPanel"))
        return;
...
function updateSubtitleSearchPanelLabels() {
