...
function initSubtitleSidebarResizer() {
    if (!resizer || !sidebar)
        return;
    if (resizer.dataset.sidebarResizeInitialized === "true")
        return;
...
function getSubtitleContextSelection(currentIdx = null) {
    ...
    const selection = buildSubtitleContextSelection(subtitles, range.currentIdx, range.backDepth, range.forwardDepth);
...
function setSubtitleContextDepths({ backDepth = subtitleContextBackDepth, forwardDepth = subtitleContextForwardDepth } = {}) {
...
function initSubtitleSearchPanel() {
    ensureSubtitleSearchPanel(sidebar, {
...
function findSubtitleTextMatches(query) {
    return findSubtitleTextMatchesInCues(subtitles, query, (text) => tokenizeJapaneseTextSync(text) || []);
}
function getSubtitleSearchHaystack(text) {
    return getSubtitleSearchHaystackForText(text, (value) => tokenizeJapaneseTextSync(value) || []);
}
...
function scrollToSearchMatch(match) {
    if (!match)
        return;
    const el = sidebar.querySelector(`.subtitle[data-index="${match.subtitleIndex}"]`);
    if (!el)
        return;
    el.scrollIntoView({
        block: "center",
...
function initSubtitleContextDrag() {
    if (document.body.dataset.subtitleContextDragInitialized === "true")
        return;
...
function onSubtitleContextDragMove(event) {
    ...
    if (!subtitleContextDragState.activated) {
        if (distanceY < dragDeadZonePx)
            return;
        subtitleContextDragState.activated = true;
    }
    updateSubtitleContextDepthFromPointer(subtitleContextDragState.kind, event.clientY, subtitleContextDragState.currentIdx);
}
...
function renderSubtitles() {
    ...
    subtitles.forEach((sub, idx) => {
        ...
        const timeContainer = createSubtitleTimeContainer(sub.start + globalSubDelay, sub.end + globalSubDelay);
