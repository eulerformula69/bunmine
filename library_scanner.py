...
EPISODE_PATTERNS = [
    re.compile(r"[Ss](\d{1,2})[Ee](\d{1,3})(?:\D|$)"),
    re.compile(r"(?:^|\D)(\d{1,3})(?:v\d+)?(?:\D|$)"),
]
...
def clean_series_title(value: str) -> str:
    ...
    # Убираем группы релизеров/технические блоки в скобках
    text = re.sub(r"\[[^\]]*\]", " ", text)
    text = re.sub(r"\([^\)]*\)", " ", text)
...
def detect_episode(path: Path) -> tuple[Optional[int], Optional[float]]:
    ...
    # Формат [01], [02], [13]
    # Берём первый числовой bracket-блок, но игнорируем технические штуки типа [1080p].
    bracket_numbers = re.findall(r"\[(\d{1,3})(?:v\d+)?\]", name, flags=re.IGNORECASE)
    for raw_number in bracket_numbers:
        number = int(raw_number)

        # 1080 не попадёт из-за \d{1,3}, но 720 мог бы попасть.
        # Серии обычно 1-200, а 720 — нет.
        if 1 <= number <= 200:
            return None, float(number)

    # Формат "Title - 01", "Title 01", "Episode 01", "EP01"
    dash_number = re.search(
...
def infer_series_title(media_root: Path, path: Path) -> str:
    ...
    candidate_parts = list(relative.parts[:-1])

    for part in candidate_parts:
        normalized_part = normalize_title(part)

        # Пропускаем папки типа "1", "2", "01"
        if normalized_part.isdigit():
            continue

        # Пропускаем общие служебные папки
        if normalized_part in ignored_folder_names:
            continue
    ...
    # fallback: если нормальной папки нет, пробуем имя файла
    name = path.stem
...
def build_normalized_episode_key(
    ...
    if episode_number is None:
        # fallback, чтобы не склеивать разные неизвестные файлы в одну серию
        return f"{normalized_series_title}|unknown|{normalize_title(path.stem)}"

    episode_part = f"{episode_number:g}"

    return f"{normalized_series_title}|s{season_part}|e{episode_part}"


def get_or_create_series(conn, title: str) -> int:
    normalized_title = normalize_title(title)
    sort_title = normalized_title

    row = conn.execute(
        "SELECT id FROM series WHERE normalized_title = ?",
        (normalized_title,),
    ).fetchone()
    ...
    if row:
        conn.execute(
            """
            UPDATE series
            SET title = ?, sort_title = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
            """,
            (title, sort_title, row["id"]),
        )
        return int(row["id"])

    cur = conn.execute(
        """
        INSERT INTO series(title, normalized_title, sort_title)
        VALUES(?, ?, ?)
        """,
...
def get_or_create_episode(
    ...
    row = conn.execute(
        "SELECT id FROM episodes WHERE normalized_key = ?",
        (normalized_key,),
    ).fetchone()
...
def scan_library(
    ...
    if not media_root.exists():
        return {
            "ok": False,
            "error": f"MEDIA_LIBRARY_DIR does not exist: {media_root}",
        }

    if not media_root.is_dir():
        return {
            "ok": False,
            "error": f"MEDIA_LIBRARY_DIR is not a directory: {media_root}",
        }

    allowed_extensions = {
        ext.lower() for ext in video_extensions | subtitle_extensions
    }
    ...
    with get_db(db_path) as conn:
        # Временно считаем все ранее известные файлы missing.
        # Всё найденное ниже сразу станет file_exists = 1.
        conn.execute(
            """
            UPDATE library_files
            SET
        ...
        for path in found_files:
            ...
            normalized_key = build_normalized_episode_key(
